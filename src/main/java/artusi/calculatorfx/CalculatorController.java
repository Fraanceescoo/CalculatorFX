...
public class CalculatorController {
    ...
    public void keyPressed(ActionEvent event){
        if (event.getSource() == btnDeleteAll) { clearOutput(); return; }
        if (event.getSource() == btnDelete) { removeLast(); return; }
        if (event.getSource() == btnAdd) { add(); return; }
        if (event.getSource() == btnSub) { sub(); return; }
        if (event.getSource() == btnMulti) { multi(); return; }
        if (event.getSource() == btnDiv) { div(); return; }
        if (event.getSource() == btnPow) { pow(); return; }
        if (event.getSource() == btnEquals) { equals(); return; }
        if (event.getSource() == btnOpenBracket) { openBrackets(); return; }
        if (event.getSource() == btnCloseBracket) { closeBrackets(); return; }
